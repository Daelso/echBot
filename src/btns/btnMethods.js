...
const recruitToStaff = async (interaction, info) => {
  ...
  const recruitEmbed = new EmbedBuilder()
    .setTitle("A new recruit has appeared!")
    .setDescription(
      `<@${recruitId}> has applied as a new recruit, please select an option below.`
    )
    .setColor("Random")
    .setTimestamp()
  ...
  staffActionRow.components.push(
    new ButtonBuilder()
      .setCustomId(`grantRecruit-${recruitId}`)
      .setLabel("Grant Recruit Role")
      .setStyle(ButtonStyle.Primary)
  );
  staffActionRow.components.push(
    new ButtonBuilder()
      .setCustomId(`delay-${recruitId}`)
      .setLabel("Be With Them Shortly")
      .setStyle(ButtonStyle.Secondary)
  );
  staffActionRow.components.push(
    new ButtonBuilder()
      .setCustomId(`deny-${recruitId}`)
      .setLabel("Deny")
...
const ambassadorToStaff = async (interaction) => {
  ...
  const staffActionRow = new ActionRowBuilder();
  const recruitEmbed = new EmbedBuilder()
    .setTitle("A potential ally is reaching out!")
    .setDescription(
      `<@${recruitId}> has applied as an Allied Ambassador, please select an option below.`
    )
    .setColor("Random")
    .setTimestamp()
    .setThumbnail("https://www.echclan.net/img/ECHLogo.73a81d16.png")
  ...
  staffActionRow.components.push(
    new ButtonBuilder()
      .setCustomId(`grantAmbassador-${recruitId}`)
      .setLabel("Grant Ambassador Role")
      .setStyle(ButtonStyle.Primary)
  );
  staffActionRow.components.push(
    new ButtonBuilder()
...
const attacheToStaff = async (interaction) => {
  ...
  const staffActionRow = new ActionRowBuilder();
  const recruitEmbed = new EmbedBuilder()
    .setTitle("An allied attache needs assistance!")
    .setDescription(
      `<@${recruitId}> is looking to receive the allied attache role, please select an option below.`
    )
    .setColor("Random")
    .setTimestamp()
    .setThumbnail("https://www.echclan.net/img/ECHLogo.73a81d16.png")
  ...
  staffActionRow.components.push(
    new ButtonBuilder()
      .setCustomId(`grantAttache-${recruitId}`)
      .setLabel("Grant Attache Role")
      .setStyle(ButtonStyle.Primary)
  );
  staffActionRow.components.push(
    new ButtonBuilder()
...
const ncapAttacheToStaff = async (interaction) => {
  ...
  const staffActionRow = new ActionRowBuilder();
  const recruitEmbed = new EmbedBuilder()
    .setTitle("A coalition attache needs assistance!")
    .setDescription(
      `<@${recruitId}> is looking to receive the coalition attache role, please select an option below.`
    )
    .setColor("Random")
    .setTimestamp()
    .setThumbnail("https://www.echclan.net/img/ECHLogo.73a81d16.png")
  ...
  staffActionRow.components.push(
    new ButtonBuilder()
      .setCustomId(`grantNCAPAttache-${recruitId}`)
      .setLabel("Grant Coalition Attache Role")
      .setStyle(ButtonStyle.Primary)
  );
  staffActionRow.components.push(
    new ButtonBuilder()
...
const beWithYouSoon = async (interaction, recruitId) => {
  await interaction.guild.channels
    ...
    .then((channel) => {
      channel.send({
        content: `<@${recruitId}>, <@${interaction.user.id}> has seen your application and will be in touch with you shortly!`,
      });
    })
    .catch((err) => {
      console.log(err);
...
const denied = async (interaction, recruitId) => {
  ...
  await interaction.guild.channels
    ...
    .then((channel) => {
      channel.send({
        content: `<@${recruitId}>, <@${interaction.user.id}> has DENIED your application. Thank you for applying.`,
      });
    })
    .catch((err) => {
      console.log(err);
  ...
  await interaction.guild.channels
    ...
    .then((channel) => {
      channel.send({
        content: `<@${interaction.user.id}> has DENIED <@${recruitId}>'s application.`,
      });
    })
    .catch((err) => {
      console.log(err);
...
const grantRecruit = async (interaction, recruitId) => {
  ...
  try {
    if (hasRole) {
      interaction.message.delete();
      staffChannel.send({
        content: `<@${recruitId}> already has the recruit role!`,
      });
    }
...
const grantAmbassador = async (interaction, recruitId) => {
  ...
  try {
    if (hasRole) {
      interaction.message.delete();
      staffChannel.send({
        content: `<@${recruitId}> already has the ambassador role!`,
      });
    }

    await recruit.roles.add(ambassadorRoleId);
    staffChannel.send({
      content: `<@${interaction.user.id}> has granted <@${recruitId}> the role of ambassador.`,
    });
  } catch (error) {
    console.log(error);
  }
...
const grantAttache = async (interaction, recruitId) => {
  ...
  try {
    if (hasRole) {
      interaction.message.delete();
      staffChannel.send({
        content: `<@${recruitId}> already has the attache role!`,
      });
    }

    await recruit.roles.add(attacheRoleId);
    staffChannel.send({
      content: `<@${interaction.user.id}> has granted <@${recruitId}> the role of attache.`,
    });
  } catch (error) {
    console.log(error);
  }
